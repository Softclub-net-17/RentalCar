...
public class ReservationController(
    ...
    ICommandHandler<ReservationUpdateCommand, Result<string>> updateHandler,
    ICommandHandler<ReservationDeleteCommand, Result<string>> deleteHandler)
    : ControllerBase
{
    [HttpGet]
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> UpdateAsync(int id, [FromBody] ReservationUpdateCommand command)
    {
        command.Id = id;
    ...
    {
        var result = await deleteHandler.HandleAsync(new ReservationDeleteCommand(id));
        if (!result.IsSuccess)
            return HandleError(result);
